...
from risk_analyzer import check_api_key_available, RiskAnalyzer
from utils import format_currency, format_percentage, format_multiple
...
if st.session_state.model_results and api_available:
    ...
    if generate_report:
        with st.spinner("Generating comprehensive IC report..."):
            try:
                from scoring import ICScoring, ScoringInputs
                ...
                scoring_inputs = ScoringInputs(
                    ...
                    max_debt_to_ebitda=results['leverage_ratios']['Debt/EBITDA'].max() if 'leverage_ratios' in results else debt_to_ebitda,
                    min_interest_coverage=results['leverage_ratios']['Interest Coverage'].min() if 'leverage_ratios' in results else 999,
                ...
                revenue_data = results['operating_model']['Revenue'].to_dict()
                chart_paths['revenue'] = chart_gen.create_revenue_projection_chart(revenue_data, company_name)
                
                # EBITDA projection chart
                ebitda_data = results['operating_model']['EBITDA'].to_dict()
                chart_paths['ebitda'] = chart_gen.create_ebitda_projection_chart(ebitda_data, company_name)
                
                # Debt schedule chart
                debt_data = results['debt_schedule']['Ending Debt'].to_dict()
                chart_paths['debt'] = chart_gen.create_debt_schedule_chart(debt_data, company_name)
                
                # Leverage chart
                leverage_data = results['leverage_ratios']['Debt/EBITDA'].to_dict()
                chart_paths['leverage'] = chart_gen.create_leverage_chart(leverage_data, company_name)
                
                # FCF waterfall
                fcf_data = results['operating_model']['FCF'].to_dict()
                chart_paths['fcf'] = chart_gen.create_fcf_waterfall(fcf_data, company_name)
                
                # Sensitivity heatmap (if available)
                if 'sensitivity_df' in st.session_state:
                    sensitivity_df = st.session_state.sensitivity_df
                    chart_paths['sensitivity'] = chart_gen.create_sensitivity_heatmap(sensitivity_df, company_name)
                
                # Step 3: Prepare context
                st.info("🤖 Generating AI-powered report sections...")
                
                # Calculate contribution analysis
                from scoring import calculate_contribution_analysis
                contribution = calculate_contribution_analysis(
                    entry_ev=results['sources_uses']['Total Sources'],
                    exit_ev=results['exit_results']['exit_ev'],
                    entry_debt=results['sources_uses']['Debt'],
                    exit_debt=results['debt_schedule']['Ending Debt'].iloc[-1],
                    entry_ebitda=entry_ebitda,
                    exit_ebitda=results['operating_model']['EBITDA'].iloc[-1],
                    entry_multiple=entry_multiple
                )
                ...
                financial_data = {
                    ...
                    'operating_model': results['operating_model'].to_dict('records'),
